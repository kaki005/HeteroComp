...
class Regime(object):
    ...
    def log_likelihood(
        ...
        llh = 0
        self.logger.info(f"{self.counterM[0].sum(axis=1)=}")
...
class CubeScope(Basemodel):
    def __init__(
        ...
        self.logger: logging.Logger = logging.getLogger(f"{__class__}")
        ...
        self.init_params(alpha, beta)  # init alpha, beta and factors
        self.regimes = []
    ...
    def init_infer(
        ...
        for ini in range(self.l):
            ...
            for iter_ in range(n_iter):
                ...
                if self.verbose or iter_ == 0 or iter_ == (n_iter - 1):
                    ...
                    self.logger.info(f"{self.counterK=}")
        ...
        self.vscost_log = []
        # self.logger.info(f"{self.prev_distributions[0]=}")
    ...
    def infer_online(
        ...
        for iter_ in range(n_iter):
            ...
            # compute log likelihood
            llh = self.log_likelihood() / self.n_events
            if self.verbose or iter_ == 0 or iter_ == (n_iter - 1):
                self.logger.info(f"llh_{iter_ + 1}: {llh}")
                # if old_assignment is not None:
                #     self.logger.info(
                #         f"{np.count_nonzero(old_assignment == self.assignment)} / {self.assignment.shape[0]}"
                #     )
        ...
        self.logger.info(f"{self.counterK=}")
        self.assignment_hist += list(self.assignment)
        return shift_id
    ...
    def model_compressinon(self, cur_n, X):
        ...
        if self.anomaly:
            ...
            self.logger.info(f"Anomaly score : {a_score}")
    ...
    def regime_initialize(self, regime_instance):
        ...
        regime_instance.n_dims = copy.deepcopy(self.n_dims)
        regime_instance.n_modes = copy.deepcopy(self.n_modes)
        ...
        regime_instance.alpha = copy.deepcopy(self.alpha)
        regime_instance.betas = copy.deepcopy(self.betas)
        regime_instance.prev_terms = copy.deepcopy(self.prev_terms)
    ...
    def save(
        ...
        """
        super(CubeScope, self).save(outdir, tensor, regime_assignments, elapsed_times)
        np.savetxt(outdir / "assignment.txt", self.assignment_hist)
...
def _gibbs_sampling_online(
    ...
    for e, x in enumerate(X):
        ...
        Z[e] = new_topic
        counterK[new_topic] += 1
        changed_num += (pre_topic != new_topic)
        for mode_, d in enumerate(x):
            counterM[mode_][d, new_topic] += 1
    return Z, prev_terms, changed_num
