...
from _src.configs.Config import ModelConfig
import pandas as pd
...
ZERO = 1.0e-8
# REGIME_R = 3.0e-3
REGIME_R = 6.0e-1 # Preventing the creation of a regime
LAMBDA = 0.1
MAX_INI_r = 2
# MAX_INI_r = 1
SEED = 0
...
class Regime(object):
    ...
    def compute_costM(self, cnt, verbose: bool):
        ...
        if verbose:
            self.logger.info(f"each_non_zero_counts: {each_non_zero_counts}")
            self.logger.info(f"n_dims: {self.n_dims}")
            self.logger.info(f"FB: {FB}, TOL_R: {tol_r}")
...
class CyberCScope(Basemodel):
    def __init__(
        ...
        self.n_dims = np.full(
            1 + len(self.categorical_idxs) + len(self.continuous_idxs), 2
        )
        self.n_dims[0] = width
        ...
        self.logger.info("##tensor dimensions##")
        self.logger.info(self.n_dims)

        self.cur_n = 0

        self.init_params(config.alpha, config.beta)  # init alpha, beta and factors
        self.regimes = []
    ...
    def init_params(self, alpha, beta):
        ...
        self.mn_factors = [
            np.full((d, self.k), 1, dtype=float)
            for d in self.n_dims[: len(self.categorical_idxs) + 1]
        ]
        self.gm_factors = [
            np.full((2, self.k), 1, dtype=float) for _ in self.continuous_idxs
        ]
    ...
    def init_status(self, tensor):
        ...
        self.counterM = [
            np.zeros((self.n_dims[d], self.k), dtype=int)
            for d, _ in enumerate([self.time_idx] + self.categorical_idxs)
        ]
        self.suffstatsMc = []
    ...
    def init_infer(
        ...
        self.prev_distributions = [
            np.full((self.l, d, self.k), 1, dtype=float)
            for i, d in enumerate(self.prev_dims)
        ]
        # self.prev_suffstats = [
        ...
        if self.l < 1:
            self.logger.info(
                "Data[:init_len] does not have enough data for initialization"
            )
            self.logger.info(
                "Inlier records in data[:init_len] must be longer than width"
            )
            exit()
        ...
        for ini in range(self.l):
            ...
            if self.verbose:
                self.logger.info("Gibbs Sampling")
            self.each_samp_llh = []
            best_llh = -np.inf
            ...
            for iter_ in range(n_iter):
                ...
                if (self.early_stoppping) and (len(self.each_samp_llh) > 1) and(
                    self.each_samp_llh[len(self.each_samp_llh)-2] >= self.each_samp_llh[len(self.each_samp_llh)-1]
                ):
                    break
    ...
    def model_initialization(
        ...
        * determine optimal l
        ...
        for ini in range(self.l):
            ...
            if self.verbose:
                self.logger.info("Online Gibbs Sampling")
            self.logger.info(f"# of events: {cnt}")
            self.each_samp_llh = []
            best_llh = -np.inf
            for iter_ in range(n_iter):
                self.assignment, self.prev_terms, change_num = self.sample_topic_online(
                    tensor.to_numpy()
                )
                ...
                if (self.early_stoppping) and len(self.each_samp_llh) > 1 and (
                    ...
                    break
                if self.early_stoppping and change_num <= 0:
                    break
                if (self.keep_best_factors) and (llh > best_llh):
                    best_counterM = copy.deepcopy(self.counterM)
                    best_counterK = copy.deepcopy(self.counterK)
            ...
            self.logger.info(f"initialized costC for train segment #{ini}: {tmp_costC}")
            self.logger.info(f"initialized costM for train segment #{ini}: {tmp_costM}")

        if self.anomaly:
            self.aggregate_initials(
                ini_regimes
            )  # ensure the inialize set in a single regime
            self.l = 1
            ...
            for mode_ in range(self.n_modes):
                if mode_ < 1 + len(self.categorical_idxs):
                    ...
                    updated_suffstats = update_stats(
                        self.suffstatsMc[mode_ - (1 + len(self.categorical_idxs))]
                    )[0]
                    self.prev_distributions[mode_][0, :, :] = copy.deepcopy(
                        updated_suffstats
                    )
        ...
        self.l = L
        self.logger.info(f"dependencies length L:{L}")
        self.prev_distributions = [
    ...
    def determine_initial_regimeset_assignments(
        ...
        for max_r_num in range(1, MAX_INI_r + 1):
            ...
            self.logger.info(
                f"candidate regimes for initial regimes:{candidate_rgm_nums}"
            )
            ...
            if cost < min_:
                ...
                self.logger.info(f"top samllest {max_r_num - 1} regimes")
                self.logger.info(f"MAX_INI_r: {MAX_INI_r}")
                break
        # count segment
        ...
        self.logger.info(f"initial regimes: {opt_rgm_nums}")
        self.logger.info(f"cost and assignment: {min_} & {opt_alloc}")
    ...
    def infer_online(self, tensor: pd.DataFrame, n_iter=10, *args):
        # whether apply tuned parameter in initialization
        if True:
            self.init_params(self.init_alpha, self.init_beta)
        ...
        for iter_ in range(n_iter):
            ...
            if (self.early_stoppping) and len(self.each_samp_llh)> 1 and (
                ...
                break
            if self.early_stoppping and change_num / self.n_events < 0.03:
                break
            if (self.keep_best_factors) and (llh > best_llh):
                best_counterM = copy.deepcopy(self.counterM)
                best_counterK = copy.deepcopy(self.counterK)
        ...
        self.logger.info(f"decomp_time:{decomp_time}")
        ...
        self.logger.info(f"compress_time:{compress_time}")
    ...
    def log_likelihood(
        ...
        for mode_, _ in enumerate(self.continuous_idxs):
            ...
            # self.logger.info(f"mode:{mode_}")
            # self.logger.info(f"shapes:{shapes}\nscales:{scales}")
            for i in range(self.k):
                N = suffstats[5, i]
                S = suffstats[6, i]
                lnS = suffstats[7, i]
    ...
    def update_prev_dist(self, cnt, l):
        ...
        if cnt:
            ...
            for mode_ in range(len(self.continuous_idxs)):
                ...
                self.prev_distributions[1 + len(self.categorical_idxs) + mode_][
                    -1, :, :
                ] = copy.deepcopy(updated_suffstats)
    ...
    def update_prev_dist_init(self, cnt, l):
        ...
        # for visualize
        if False:
            self.logger.info(f"prev distributions for {l}")
            for mode_ in range(self.n_modes):
                self.logger.info(self.prev_distributions[mode_])
    ...
    def model_compressinon(self, cur_n, X):
        ...
        self.logger.info(f"cand costC: {costC}")
        self.logger.info(f"cand costM: {costM}")
        self.logger.info(f"cand costT: {cost_1}")

        cost_0 = prev_rgm.compute_costC(X)
        self.logger.info(f"prev costT: {cost_0}")

        self.vscost_log.append([cost_0, cost_1, cost_1 - cost_0, costC, costM])

        self.logger.info("=========================================")
        self.logger.info("Previous vs Candidate")
        self.logger.info(f"{cost_0} vs {cost_1}")
        self.logger.info("=========================================")
        self.logger.info(f"diff::{cost_1 - cost_0}")

        if cost_0 < cost_1:  # stay on previous regime
            self.logger.info("STAY")
            prev_rgm = self.regimes[self.prev_rgm_id]
            ...
            self.logger.info(f"SHIFT at {cur_n}")
            self.logger.info(f"{self.prev_rgm_id}===>>>{shift_id}")
        ...
        if self.anomaly:
            ...
            self.logger.info("===Anomaly score===")
            self.logger.info(a_score)
    ...
    def save(
        ...
        """
        super(CyberCScope, self).save(outdir, tensor, regime_assignments, elapsed_times)
        ...
        np.savetxt(outdir / "anomaly.txt",  self.anomaly_scores)
...
def _gibbs_sampling(
    X, Z, counterM, counterK, counterA, alpha, betas, suffstatsMc, k, n_dims
):
    # np.random.seed(SEED)
    """
    X: event tensor
    ...
    for e, x in enumerate(X):
        ...
        if not pre_topic == -1:
            ...
            for mode_ in range(n_continuous_modes):
                suffstatsMc[mode_] = decrement_Mc(
                    x[n_all_categorical_modes + mode_], pre_topic, suffstatsMc[mode_]
                )
        ...
        for j in range(n_continuous_modes):
            # self.logger.info(f"continous mode:{j}")
            # self.logger.info(pdf_k(x[n_all_categorical_modes+j], suffstatsMc[j]))
            posts *= np.clip(
                pdf_k(x[n_all_categorical_modes + j], suffstatsMc[j]), PDF_MIN, None
            )
        posts = posts / posts.sum()

        try:
            # self.logger.info("final") #TODO
            # self.logger.info(posts) #TODO
        ...
        for mode_ in range(n_continuous_modes):
            suffstatsMc[mode_] = increment_Mc(
                x[n_all_categorical_modes + mode_], new_topic, suffstatsMc[mode_]
            )
    return Z
...
def _gibbs_sampling_online(
    ...
    for e, x in enumerate(X):
        ...
        Z[e] = new_topic
        counterK[new_topic] += 1
        change_num += new_topic != pre_topic
        for mode_ in range(n_all_categorical_modes):
            counterM[mode_][int(x[mode_]), new_topic] += 1
        for mode_ in range(n_continuous_modes):
            suffstatsMc[mode_] = increment_Mc(
    ...
    return Z, prev_terms, change_num


# endregion (CyberCScope)
    ...
    for x in X:
        ...
        for r in range(k):
            ...
            for att_value, factor in zip(x[n_all_categorical_idxs:], gm_factors):
                ...
                rval_ += llh_gamma(
                    1, att_value + ZERO, np.log(att_value + ZERO), shape, scale
                )
...
def decrement_Mc(x, z, suffstats, eps=1e-10):
    ...
    suffstats[9, z] = (suffstats[5, z] * (suffstats[9, z] + mu_p**2) - x**2) / (
        suffstats[5, z] - 1
    ) - suffstats[8, z] ** 2
    suffstats[5, z] -= 1
    suffstats[6, z] -= x
...
def get_params(suffstats_z):
    ...
    # self.logger.info("shape,scale")
    # self.logger.info(shape_hat,scale_hat)
...
def update_stats(suffstats_z):
    ...
    shape_hat = approximation(
        shape_hat, ln_a_hat, b_hat, c_hat, suffstats_z[3], np.log(e_hat), suffstats_z[5]
    )
    # self.logger.info(f"shape_hat:{shape_hat}")
...
def approx_digamma(x, eps=1e-10):
    ...
    value = value - r * (
        1.0 / 12.0
        - r * (1.0 / 120.0 - r * (1.0 / 252.0 - r * (1.0 / 240.0 - r * (1.0 / 132.0))))
    )
    return value
...
def gamma_pdf(x, alpha, theta):
    ...
    log_pdf = (
        (alpha - 1) * np.log(x) - (x / theta) - log_gamma_alpha - alpha * np.log(theta)
    )
