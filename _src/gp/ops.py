...
from .bayesnewton_utils import mvn_logpdf, process_noise_covariance, solve


def cholesky_tridiagonal(diag: Float[Array, "N"], subdiag: Float[Array, "N-1"]) -> tuple[Float[Array, "N"], Float[Array, "N-1"]]:
    """
...
def kalman_filter_online(dt, kernel, y, noise_cov, m0, P0, mask=None, parallel=False, return_predict=False):
    """
    ...
    if parallel:
        ell, means, covs = _parallel_kf(As, Qs, H, y, noise_cov, m0, P0, mask, return_predict=return_predict)
    else:
        ell, means, covs, Ss, vs = _sequential_kf(As, Qs, H, y, noise_cov, m0, P0, mask, return_predict=return_predict)
...
def _sequential_kf(As, Qs, H, ys, noise_covs, m0, P0, masks, return_predict=False):
    def body(carry, inputs):
        ...
        m, P, ell = carry
        m_ = A @ m
        P_ = A @ P @ A.T + Q  # predict cov

        obs_mean = H @ m_
        HP = H @ P_
        S = HP @ H.T + obs_cov  # innovation variance
        v = y - obs_mean  # innvoation mean
    ...
    (_, _, loglik), (fms, fPs, Ss, vs) = scan(f=body, init=(m0, P0, 0.0), xs=(ys, As, Qs, noise_covs, masks))
