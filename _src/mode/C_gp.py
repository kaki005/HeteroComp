...
from _src.gp import BaseModel, LogDensityMarkovGP, MarkovGP, Matern32, Poisson, Trainer, weighted_logsumexp
...
class C_gp(Base):
    def __init__(
        ...
        self.func_dim: int = 2
        super(C_gp, self).__init__(k, self.func_dim + self.func_dim * self.func_dim, config)
        ...
        self.bin_x: Float[np.ndarray, "num_bins"] = np.mean(bounds, axis=1).reshape(-1, 1)
        self.leggaussX, self.leggaussW = np.polynomial.legendre.leggauss(300)
        self.leggaussX = self.leggaussX.reshape(-1, 1)
        self.grid_bounds: Float[np.ndarray, "num_bins 2"] = bounds
        self.grid_size: Float[np.ndarray, "num_bins"] = np.diff(bounds, axis=1).squeeze()
    ...
    def compute_posterior(self, counterM: Float[np.ndarray, "mode k"], counterK: Float[np.ndarray, "K"]) -> None:
        """
        ...
        """
        self.states, self.post_mean, self.post_var = _compute_posterior(self.gp, self.states, counterM)
        self.mode_prob = np.array(_mode_prob_LDGP(self.post_mean, self.post_var, self.grid_size))
    ...
    def init_gibbs(self, tensor: pd.DataFrame, init_gp: bool = False):
        ...
        if init_gp:
            ...
            gp, states = _createGP(np.arange(self.k), self.bin_x, self.grid_size, self.config.C_lengthscale)
            self.gp = gp
            self.states = states
    ...
    def update_hyperparam(self, counterM, counterK):
        ...
        self.gp = self.trainers.update(grad)

    def update_prev_dist(self, counterM: np.ndarray, counterK: np.ndarray):
        self.compute_posterior(counterM, counterK)
        return 0

    def save(self, out_dir: pathlib.Path, counterM: np.ndarray):
        np.savetxt(out_dir / "final_mean.txt", self.post_mean.squeeze().T)
        np.savetxt(out_dir / "final_var.txt", self.post_var.squeeze().T)
        np.savetxt(out_dir / "final_prob.txt", self.mode_prob.squeeze())
        if len(self.means_history) > 0:
            np.savetxt(out_dir / f"mean_{self.mode_idx}_hist.txt", np.vstack(self.means_history))
        if len(self.vars_history) > 0:
            np.savetxt(out_dir / f"var_{self.mode_idx}_hist.txt", np.vstack(self.vars_history))
        save_sparse_matrix(
            out_dir / "counter.csv",
            counterM,
            "bin",
    ...
    def save_online(self, output_path: pathlib.Path, counterM: Int[np.ndarray, "dim k"]):
        np.savetxt(output_path / f"mean_{self.mode_idx}.txt", self.post_mean.squeeze().T)
        ...
        save_sparse_matrix(output_path / f"counter_{self.mode_idx}.csv", counterM, "grid", "topic", self.grid_labels)

    def plot(self, out_dir: pathlib.Path, counterM: np.ndarray):
        # plot latent gp
        _plot_latent_gp(out_dir / "latent_gp.png", self.bin_x, self.colors, self.post_mean, self.post_var, self.grid_bounds)
        # counter
        _plot_histgram(out_dir / "counter.png", counterM, self.bin_x, self.colors, self.mode_idx)
    ...
    def log_likelihood_init(self, counterM: Int[np.ndarray, "dim k"], counterK: Float[np.ndarray, "K"]):
        """log likelihood in the initial phase.
    ...
    def log_likelihood(self, counterM: Int[np.ndarray, "dim k"], counterK: Int[np.ndarray, "K"]) -> np.ndarray:
        """log likelihood.
    ...
    @override
    def _compute_prev_dist(self, counterM: Int[np.ndarray, "dim k"], counterK: Int[np.ndarray, "k"]) -> Float[np.ndarray, "k"]:
        return np.zeros(1)
...
def _createGP(index, x: np.ndarray, bin_sizes: Float[Array, "num_bins"], lengthscale: int) -> tuple[LogDensityMarkovGP, eqx.nn.State]:
...
def _compute_posterior(gp: LogDensityMarkovGP, state: State, Y: np.ndarray):
    ...
    def _update(state):
        (mean, var, jacobian, hessian, state), (diff1, diff2) = gp.inference(state, LR_NEWTON, Y=Y)
...
def _mode_prob_LDGP(
    ...
    log_normalizer = weighted_logsumexp(means.squeeze(), grid_sizes.squeeze(), axis=0)  # \log [\sum_n^grid exp(\lambda_{nk}) grid_sizes_n]
    ...
    for k in range(K):
        for i in range(2):
            for bound in grid_bounds:
                ax[k, i].axvline(x=bound[0], linestyle="--", color="gray", linewidth=1)
...
def _plot_histgram(path: pathlib.Path, counterM, bin_x: np.ndarray, colors, mode_idx: str):
    ...
    K = counterM.shape[1]
    for k in range(K):
        ax[0].bar(np.arange(num_bins), counterM[:, k], color=colors[k], bottom=offset, label=f"topic {k + 1}")
        offset += counterM[:, k]
